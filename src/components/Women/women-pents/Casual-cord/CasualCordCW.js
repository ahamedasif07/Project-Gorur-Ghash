...
const CasualCordCW = () => {
  ...
  const [WomenCord] = useState("women-casual-cord-pants");
    ...
    if (allProductsData.length > 0) {
      const remaining = allProductsData.filter(
        (product) =>
          ...
          WomenCord.toLowerCase().trim()
      );
      setFiltaredProducts(remaining);
    }
  }, [allProductsData, WomenCord]);
